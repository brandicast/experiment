...
var eventCode = {4001:"Tamper trigger", 4002:"Low battery", 4101:"PIR Trigger",  4102:"Door/Window Open", 4103:"Door/Window Close", 4801:"Temperature Report", 4802:"Illumination report", 4804:"Meter report", 5002:"Status update"};
...
function filter_philio_data (x){
    ...
    {
        var now = Date.now() ;
        console.log ('1) current timeStamp  -  last_record.temp_timestamp  = ' +  now  + "-" +  last_record.temp_timestamp) ;
        ...
        switch (x.eventlog.funcType){
            case 11 : {
                ...
                if ((now -  last_record.temp_timestamp ) >  period) {
                    last_record.temp_timestamp = now ; 
                    message = x.eventlog.uid + ": " + "現在溫度 =  " + last_record.temp ;
                }
                break ;
            }
            case 12 :  {            
                last_record.lumin = x.eventlog.sensorValue; 
                if ((now - last_record.lumin_timestamp  ) >  period) {
                    last_record.lumin_timestamp = now ;
                    message = x.eventlog.uid + ": " + "現在亮度 =  " +  last_record.lumin  ;
                }
                break ;
            }   
            case 0 : {
...
setTimeout(function(){
    bot.push (user_id_array, "現在溫度 : " + last_record.temp + ", 現在亮度 :" + last_record.lumin ) ;
