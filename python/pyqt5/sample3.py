...
class Example(QWidget):
    ...
    def __init__(self):
        ...
        self.tree.addTopLevelItem(QTreeWidgetItem(['Item 1']))
        self.tree.addTopLevelItem(QTreeWidgetItem(['Item 2']))
        self.tree.addTopLevelItem(QTreeWidgetItem(['Item 3']))
    ...
    def mouseMoveEvent(self, event):
        if self.__isResizing:
            ...
            self.tree.resize(self.tree.width() + diff.x(),
                             self.tree.height() + diff.y())
            self.__mouseMovePos = event.globalPos()
            self.adjustSize()
